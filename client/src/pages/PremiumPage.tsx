...
export default function PremiumPage() {
  ...
  const openPortal = useMutation({
    meta: { silenceToast: true },
    mutationFn: () =>
      apiFetch<{ url: string }>("/api/premium/portal", { method: "POST" }),
    onSuccess: ({ url }) => {
      ...
      <div className="space-y-8">
        ...
        <section>
          <h2 className="text-sm font-medium mb-3">
            {t("premium.catalogFeed")}
          </h2>
          <div className="rounded-3xl border bg-card p-5">
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
              <div className="flex items-center gap-2">
                <span
                  className={`inline-block size-2 rounded-full ${live ? "bg-emerald-500" : "bg-muted-foreground/40"}`}
                />
              ...
              <span className="text-xs text-muted-foreground">
                {t("premium.lastChecked", {
                  when: fmtWhen(catalog.lastSyncMs) ?? t("common.never"),
                })}
              </span>
            </div>
            ...
            <div className="rounded-3xl border bg-card p-5 space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {licensed &&
                  ...
                  license.cancelAtPeriodEnd &&
                  license.expiresAt &&
                  t("premium.willNotRenew", {
                    date: fmtDate(license.expiresAt),
                  })}
              ...
              {openPortal.isError && (
                <p className="text-destructive text-xs">
                  {(openPortal.error as Error).message}
                </p>
              )}
            </div>
          ) : (
